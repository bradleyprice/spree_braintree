...
module Spree
  class Gateway::Braintree < ::Spree::Gateway
    ...
    def create_profile(payment)
      ...
      address = (payment.source.try(:address) || payment.order.bill_address).try(:active_merchant_hash)
      ...
      if result.success?
        ...
        source.tap do |spree_cc|
          if card.is_a?(::Braintree::PayPalAccount)
            spree_cc.cc_type = 'paypal'
            data = {
              email: card.email
            }
            spree_cc.data = data.to_json
          else
            spree_cc.name = card.cardholder_name
            spree_cc.cc_type = CARD_TYPE_MAPPING[card.card_type]
          ...
          spree_cc.gateway_customer_profile_id = result.customer.id
